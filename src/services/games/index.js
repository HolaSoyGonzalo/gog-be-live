...
router.post("/", cloudMulter.single("cover"), async (req, res, next) => {
  try {
    ...
    //writeFile(currentGames) this last 2 comments can be resumed in the next line of code
...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    const remainingList = fullList.filter((game) => game.id !== req.params.id); // we exclude from the list the element we want to update
    const toUpdate = fullList.find((game) => game.id === req.params.id); //we take that element
    if (!toUpdate) {
      // If element does not exist => 404
    ...
    await writeFile([
      //write down the elements without the one we are updating
      ...remainingList,
      updatedGame, //followed by the previous element stitched with the properties we have in the body
    ]);
